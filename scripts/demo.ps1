...
$last = ""
while ($true) {
    $items = aws dynamodb query --table-name $t --index-name GSI1 --key-condition-expression "gsi1pk = :p" --expression-attribute-values '{\":p\":{\"S\":\"INC\"}}' --query "Items" | ConvertFrom-Json
