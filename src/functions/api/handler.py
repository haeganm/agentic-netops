...
def lambda_handler(event, context):
    ...
    try:
        ...
        if route == "GET /incidents/{id}/verify":
            return _ok(ledger.verify_ledger(path_params["id"]))
...
def _maps_to_drift(actor: str, meta: dict) -> bool:
    """Segregation of duties (ADR 0013): would this approver be signing off on drift they
    themselves caused? Resolves the Cognito-email -> IAM-ARN mismatch via CONFIG#APPROVERS."""
...
def _decide(iid: str, actor: str, approve: bool, body):
    ...
    if approve:
        ...
        # SoD: an approver may not sign off on drift they caused...
        if _maps_to_drift(actor, meta):
            return _err(403, "segregation of duties: you caused this change")
        # ...and the second approver must be DISTINCT from the first (two-party control).
